     ...
     if (!outpute ) // equivalent to saying if ( in_file == NULL ) 
            {  
                ...
                fclose(outpute);
                        ...
                        while (!feof(output))
                        { 
                                fscanf(output, "%s %s %s %s %s %d %s %s" , email[i],fname[i],lname[i],password[i],bank_account[i],
                                &balance[i],secret_question[i],reply[i]);
                                i++;
                                count++;
                        }fclose(output);
...
if (strcmp(sign,"sign in")==0)
{
    ...
    while(!feof(outputee))
    {
    fscanf(outputee,"%s %s %s %s %s %d %s %s ",email[i],fname[i],lname[i],password[i],bank_account[i],&balance[i],secret_question[i],reply[i]);
    }
    fclose(outputee);
sign_in(count,c);
}
    ...
    scanf("%s",word);
        if(strcmp(word,"order") == 0){
            ...
            if(balance[id]>total_amount){
                ...
                int i = 0,user = 0;
                FILE *teta;
                teta = fopen("output.txt","r");
                while (!feof(teta))
                {
                    fscanf(teta, "%s %s %s %s %s %d %s %s\n" , email[i],fname[i],lname[i],password[i],bank_account[i],
                &balance[i],secret_question[i],reply[i]);
                i++;
                user++;
                }
...
/* else if(strcmp(new_email,email[i])!=0 && strcmp(new_bankaccount,bank_account[i])!=0)*/
        ...
        count = count - 1;
                  for (int i = 0 ; i < count ; i++)
                {
                        fprintf(file, "%s %s %s %s %s %d %s %s\n" , email[i],fname[i],lname[i],password[i],bank_account[i],
                balance[i],secret_question[i],reply[i]);
                // the problem was here i put count ++ and i++ so the for loop didnt work well
        ...
        fclose(fptr);
        int i = 0;
        FILE *omega=fopen("output.txt","r");
        while (!feof(omega))
        {
             
                   
          fscanf(omega, "%s %s %s %s %s %d %s %s" , email[i],fname[i],lname[i],password[i],bank_account[i],
          &balance[i],secret_question[i],reply[i]);
           i++;
