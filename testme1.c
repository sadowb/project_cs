...
if (strcmp(sign, "sign in")==0)
{
    ...
    {
    if(strcmp(email[i],address) == 0||strcmp(Email[i],address))
    {
        found = 1;
        id = i;
        break;
    ...
    for(int i=0;i<count;i++){
        if (strcmp(new_email,email[i])==0 || strcmp(new_bankaccount,bank_account[i])==0 || strcmp(new_email,Email[i])==0 || strcmp(new_bankaccount,Bank_account[i])==0)//checks with the arrays if the arrays
        {
        ...
        else if(strcmp(new_email,email[i])!=0 || strcmp(new_bankaccount,bank_account[i])!=0 || strcmp(new_email,Email[i])!=0 || strcmp(new_bankaccount,Bank_account[i])!=0)
    {
        ...
        strcpy(Email[count+1],new_email);
        strcpy(Bank_account[count+1],new_bankaccount);
        strcpy(Lname[count+1],new_lname);
        strcpy(Fname[count+1],new_fname);
        strcpy(Password[count+1],new_password);
        printf("\nLog in again");
        break;
        }
    }
